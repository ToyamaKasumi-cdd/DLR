...
parser.add_argument("--alpha", type=float, default=0.5,
                    help="xxx")
parser.add_argument("-w4f", "--weight4f", type=float, default=0.2,
                    help="xxx") 
...
timestamps=len(train_inv_list+valid_inv_list+test_inv_list)

# Get model
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
model = DQGCN(timestamps, num_nodes, num_rels, args)
